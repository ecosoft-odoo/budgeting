...
def uninstall_hook(cr, registry):
    ...
    for dimension in dimensions:
        ...
        cr.execute(
            "DELETE FROM ir_model_fields WHERE name=%s and model='account.payment.register'",
            (name_column,),
        )
        cr.execute(
            "DELETE FROM ir_model_fields WHERE name=%s and model='account.payment.deduction'",
            (name_column,),
        )
