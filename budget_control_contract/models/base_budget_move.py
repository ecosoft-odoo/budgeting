...
class BudgetDoclineMixin(models.AbstractModel):
    ...
    def _update_budget_commitment(self, budget_vals, reverse=False):
        ...
        if self._budget_model() == "contract.budget.move":
            if (
                self.analytic_account_id.bm_date_from
                and budget_vals["date"] <= self.analytic_account_id.bm_date_from
            ) or (
                self.analytic_account_id.bm_date_to
                and budget_vals["date"] >= self.analytic_account_id.bm_date_to
            ):
