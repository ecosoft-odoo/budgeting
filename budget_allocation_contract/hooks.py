...
def post_init_hook(cr, registry):
    ...
    _models = env["ir.model"].search(
        [("model", "in", ["contract.budget.move", "contract.line"])], order="id"
    )
    _models.write(
        {
            "field_id": [
...
def uninstall_hook(cr, registry):
    ...
    for dimension in dimensions:
        ...
        cr.execute(
            "DELETE FROM ir_model_fields WHERE name=%s and model='contract.budget.move'",
            (name_column,),
        )
        cr.execute(
            "DELETE FROM ir_model_fields WHERE name=%s and model='contract.line'",
            (name_column,),
        )
