...
class MisBudgetAwareExpressionEvaluatorActivity(
    MisBudgetAwareExpressionEvaluator
):
    def __init__(
        self,
        ...
        super().__init__(
            date_from, date_to, kpi_data, additional_move_line_filter
        )
    ...
    def drilldown(self, arg):
        ...
        if self.report_id.is_activity:
            ...
            if period_id and expr and AEPA.has_account_var(expr):
                period = self.env["mis.report.instance.period"].browse(
                    period_id
                )
                aep = AEPA(
