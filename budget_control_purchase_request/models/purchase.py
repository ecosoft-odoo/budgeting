    ...
    def uncommit_purchase_request_budget(self):
            ...
            for pr_line in po_line.purchase_request_lines.filtered("amount_commit"):
                pairs.append(
                    (
                        ...
                        {
                            "analytic_account_id": (
                                pr_line.fwd_analytic_account_id or False
                            ),
                            "purchase_line_id": po_line.id,
                            "date": po_line.date_commit,
                        },
        ...
        for pr_line, vals in pairs:
            if pr_line.id not in to_commit_ids:
                continue
            line_vals = pr_line._prepare_commit_vals(reverse=True, **vals)
            if line_vals:
                budget_vals.append(line_vals)
                move_dates.append(pr_line.date_commit)
        if not budget_vals:
            return BudgetMove

        budget_moves = BudgetMove.create(budget_vals)
        # Template/KPI assignment is batched (grouped by period + account)
        # instead of one resolution per move. The former commit_budget() path
        # resolved the period from the source PR line's date, not the reverse
        # move's own date (set to the PO line's date_commit above), so
